...
class EntityDeaths extends Human {
    ...
    public function onUpdate(int $currentTick): bool{
        ...
        $line = TE::colorize(KFFA::getInstance()->config->get("SubTitle_deaths"));
        $footer = TE::colorize("&l&7IP&8 › &r&e". KFFA::getInstance()->config->get("IP_server"));
        $msg = $title . TE::EOL . $line . TE::EOL;
        $place = 1;
        $kills = KFFA::getDeathsAsRaw();
        arsort($kills);
        foreach($kills as $player => $kill){
            if($place > 10) break;
            $msg .= str_replace(["{place}", "{player}", "{death}"], [$place, $player, $kill], TE::colorize(KFFA::getInstance()->config->get("Line_topdeaths"))). TE::EOL;
            $place++;
        }
        $msg .= $footer;
        $this->setNameTag($msg);
